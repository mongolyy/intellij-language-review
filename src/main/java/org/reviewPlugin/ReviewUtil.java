...
public class ReviewUtil {
    ...
    static List<ReviewBlockId> findIds(Project project, String key) {
        List<ReviewBlockId> result = null;
        Collection<VirtualFile> virtualFiles =
                FileTypeIndex.getFiles(ReviewFileType.INSTANCE, GlobalSearchScope.allScope(project));
        ...
        for (VirtualFile virtualFile : virtualFiles) {
            ...
            ReviewFile reviewFile = (ReviewFile) PsiManager.getInstance(project).findFile(virtualFile);
            if (reviewFile != null) {
                Collection<ReviewBlockId> properties = PsiTreeUtil.findChildrenOfType(reviewFile, ReviewBlockId.class);
                for (ReviewBlockId blockId : properties) {
    ...
    public static List<ReviewBlockId> findIds(Project project, VirtualFile virtualFile, String key) {
        List<ReviewBlockId> result = null;
        ReviewFile reviewFile = (ReviewFile) PsiManager.getInstance(project).findFile(virtualFile);
        if (reviewFile != null) {
            Collection<ReviewBlockId> properties = PsiTreeUtil.findChildrenOfType(reviewFile, ReviewBlockId.class);
    ...
    static List<ReviewBlockId> findIds(Project project) {
        List<ReviewBlockId> result = new ArrayList<>();
        Collection<VirtualFile> virtualFiles =
                FileTypeIndex.getFiles(ReviewFileType.INSTANCE, GlobalSearchScope.allScope(project));
        ProjectFileIndex index = ProjectRootManager.getInstance(project).getFileIndex();
        for (VirtualFile virtualFile : virtualFiles) {
    ...
    @NotNull
    public static ReviewFile createFileFromText(@NotNull Project project, @NotNull String text) {
        return (ReviewFile) PsiFileFactory.getInstance(project).createFileFromText("a.adoc", ReviewLanguage.INSTANCE, text);
    }
